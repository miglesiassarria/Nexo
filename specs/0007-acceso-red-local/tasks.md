...
- [x] **T1.** Añadir `axum-server` (`tls-rustls`) y `rcgen` a
...
- [x] **T2.** `net::detect_lan_ip()` con el truco del socket UDP sin conectar
...
- [x] **T3.** `tls_cert::ensure(data_dir)`: genera el certificado si no
...
- [x] **T4.** `gateway::serve_on_tls(nexo, listener, cert: &LanCertificate)`,
      ...
      respuesta válida; sin token, recibe 401 — igual que hoy en local; HTTP
      plano contra ese mismo puerto no obtiene respuesta válida.
  - Ficheros: `crates/nexo-core/src/gateway/mod.rs`,
    `crates/nexo-core/tests/gateway_e2e.rs` (el harness `start_with_lan_tls`
    ya existía para issuir tokens de aplicación reales, más natural que
    duplicarlo en el test unitario de T1)
  - Verificación: `cargo test -p nexo-core --test gateway_e2e -- lan_mode a_plain_http_request_to_the_tls_port` (en verde)

- [x] **T5.** `Settings::bind_addr()` devuelve `0.0.0.0:{port}` cuando
  ...
  - Verificación: `cargo test -p nexo-core -- config::` (en verde)

- [x] **T6.** `Nexo::prepare_gateway_bind(&self, settings, data_dir) ->
...
- [x] **T7.** `main.rs` usa `prepare_gateway_bind()` para decidir la
...
- [x] **T8.** Comando `lan_risk_notice() -> RiskNotice` con el aviso
...
- [x] **T9.** Interfaz: interruptor "Permitir acceso desde mi red local" en
...
- [x] **T10.** Regresión de punta a punta del criterio de aceptación 1: con
...
- [x] Verificación del repositorio: `cargo test --workspace && cargo clippy --workspace --all-targets && npm run check` — 273 tests + 28 e2e (11 ignorados por credenciales/LM Studio) en verde, clippy sin avisos, check sin errores
- [x] Aplicación de macOS compilada **e instalada**: `npm run app:install` — compilado e instalado `Aug 2 17:41:34 2026`
- [x] Criterios de aceptación de `spec.md` repasados uno por uno, con su resultado real
- [x] Documentación actualizada: `ROADMAP.md` (criterio 18 y checklist de Fase 3) marcados hechos
- [x] `specs/README.md` actualizado a `hecho`
